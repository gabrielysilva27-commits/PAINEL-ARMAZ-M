(() => {
  ...
  async function call(action,payload={}){
    ...
    if(!r.ok)throw new Error(d.error||'Erro na Ordem de Guarda');
    return d;
  }
  ...
  function shell(){
    return '<div class="put-shell">'+
      '<section class="put-kpis">'+
        '<article><span>Planejadas</span><strong id="putPlanned">—</strong></article>'+
      ...
      '<section class="put-panel">'+
        ...
        '<div class="put-note"><strong>Fluxo:</strong> Conferência NRI → Ordem de Guarda → empilhador confirma o endereço → conferente valida → Estoque x Estoque é atualizado automaticamente.</div>'+
        '<div class="put-table-wrap"><table><thead><tr><th>Ordem</th><th>Recebimento</th><th>Status</th><th>Paletes</th><th>Pendentes</th><th>Validados</th><th>Ação</th></tr></thead><tbody id="putBody"></tbody></table></div>'+
        '<div id="putEmpty" class="put-empty hidden">Nenhuma conferência concluída disponível para Ordem de Guarda.</div>'+
      '</section>'+
      '<dialog id="putDialog" class="put-dialog"><div class="put-dialog-inner"><div class="put-dialog-head"><div><p class="eyebrow">ORDEM DE GUARDA</p><h2 id="putDialogTitle"></h2><p id="putDialogMeta"></p></div><button id="putClose" class="put-close">×</button></div><div class="put-dialog-actions"><button class="outline-button" id="putGuide">Abrir guia do empilhador</button><button class="outline-button" id="putRefreshPlan">Recalcular sugestões</button><button class="primary-button" id="putPrint">Imprimir Ordem de Guarda</button></div><div id="putDialogBody"></div></div></dialog>'+
    '</div>';
  }
  ...
  async function open(){
    ...
    $('pageTitle').textContent='Recebimento · Guarda';
    $('pageSubtitle').textContent='Endereçamento dirigido após a conferência, sem alterar o papel da NRI.';
    if(!root.dataset.mounted){
      root.innerHTML=shell();root.dataset.mounted='1';
      $('putRefresh').onclick=load;
  ...
  async function load(){
    ...
    body.innerHTML='<tr><td colspan="7" class="put-loading">Carregando ordens...</td></tr>';
    try{
      ...
      const receipts=(r.receipts||[]).filter(x=>x.status==='conference_completed');
      const rows=[];
      for(const rec of receipts){
        ...
        if(ord){
          ...
          rows.push('<tr><td><strong>'+esc(ord.order_code)+'</strong><small>'+fd(ord.created_at)+'</small></td>'+
            ...
            '<td><span class="put-status '+esc(ord.status)+'">'+esc(statusLabel(ord.status))+'</span></td>'+
          ...
          rows.push('<tr><td><strong>—</strong></td><td><strong>'+esc(rec.display_name||rec.receipt_code)+'</strong><small>'+esc(rec.receipt_code)+' · '+fd(rec.arrival_date)+'</small></td>'+
            '<td><span class="put-status not-created">Não gerada</span></td><td>'+Number(rec.nri_count||0)+'</td><td>—</td><td>—</td>'+
      ...
      const all=orders;
      $('putPlanned').textContent=all.filter(x=>x.status==='planned').length;
      $('putProgress').textContent=all.filter(x=>x.status==='in_progress').length;
      $('putPending').textContent=all.filter(x=>x.status==='awaiting_validation').length;
      $('putDone').textContent=all.filter(x=>x.status==='completed').length;
      body.querySelectorAll('[data-open-order]').forEach(b=>b.onclick=()=>openOrder(Number(b.dataset.openOrder)));
      body.querySelectorAll('[data-generate]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{const d=await call('get_or_create',{receipt_id:Number(b.dataset.generate)});await showOrder(d.order);await load()}catch(e){showToast(e.message,true)}finally{b.disabled=false}});
    }catch(e){
      body.innerHTML='<tr><td colspan="7" class="put-error">'+esc(e.message)+'</td></tr>';
    }
  }
  ...
  function printOrder(order){
    ...
    const html='<!doctype html><html><head><meta charset="utf-8"><title>'+esc(order.order_code)+'</title><style>'+
      '@page{size:A4 portrait;margin:12mm}body{font-family:Arial,sans-serif;color:#111;font-size:10pt}h1{font-size:18pt;margin:0 0 4mm}.meta{display:grid;grid-template-columns:repeat(2,1fr);gap:2mm 8mm;margin-bottom:5mm}.meta div{border-bottom:1px solid #bbb;padding:2mm 0}.notice{border:1px solid #999;padding:3mm;margin:4mm 0;font-size:9pt}table{width:100%;border-collapse:collapse}th,td{border:1px solid #999;padding:2.2mm;text-align:left;vertical-align:top}th{background:#eee;font-size:8pt;text-transform:uppercase}td{font-size:9pt}small{font-size:7.5pt}.sign{margin-top:7mm;display:grid;grid-template-columns:1fr 1fr;gap:12mm}.sign div{border-top:1px solid #444;padding-top:2mm;text-align:center}.link{font-size:7pt;word-break:break-all;margin-top:5mm}'+
      '</style></head><body><h1>ORDEM DE GUARDA · '+esc(order.order_code)+'</h1>'+
      ...
      '<div class="sign"><div>Empilhador</div><div>Conferente</div></div><div class="link"><b>Guia digital:</b> '+esc(order.public_url)+'</div></body></html>';
