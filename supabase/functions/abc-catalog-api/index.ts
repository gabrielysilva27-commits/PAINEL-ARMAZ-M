...
async function readAllCatalog(){
  const out:any[]=[];
  const page=1000;
  for(let from=0;;from+=page){
    const {data,error}=await db.from("product_catalog")
      .select("sku_code,sku_name,factor_hecto_commercial,boxes_per_pallet,source_file,updated_at")
      .order("sku_code")
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==="status"){
      const [{count:total,error:e1},{count:withFactor,error:e2}]=await Promise.all([
        db.from("product_catalog").select("sku_code",{count:"exact",head:true}),
        db.from("product_catalog").select("sku_code",{count:"exact",head:true}).gt("factor_hecto_commercial",0)
      ]);
      if(e1)throw e1;if(e2)throw e2;
      return json({count:total||0,valid_factor_count:withFactor||0,no_factor_count:Math.max(0,(total||0)-(withFactor||0))});
    }
    if(action==="get"){
      const data=await readAllCatalog();
      const valid=data.filter((x:any)=>Number(x.factor_hecto_commercial)>0).length;
      return json({items:data,count:data.length,valid_factor_count:valid,no_factor_count:data.length-valid});
    }
    if(action==="import"){
      if(user.role!=="admin")return json({error:"Sem autorização"},403);
      const items=Array.isArray(body.items)?body.items:[];
      if(!items.length)return json({error:"Nenhum produto recebido"},400);
      let saved=0,withFactor=0;
      for(let i=0;i<items.length;i+=200){
        const batch=items.slice(i,i+200).map((x:any)=>{
          const factor=n(x.factor_hecto_commercial);
          return {
            sku_code:String(x.sku_code||"").trim(),
            sku_name:String(x.sku_name||""),
            factor_hecto_commercial:factor&&factor>0?factor:null,
            boxes_per_pallet:n(x.boxes_per_pallet),
            source_file:String(body.source_file||"01.11"),
            updated_at:new Date().toISOString()
          };
        }).filter((x:any)=>x.sku_code);
        if(!batch.length)continue;
        const {error}=await db.from("product_catalog").upsert(batch,{onConflict:"sku_code"});
        if(error)throw error;
        saved+=batch.length;
        withFactor+=batch.filter((x:any)=>Number(x.factor_hecto_commercial)>0).length;
      }
