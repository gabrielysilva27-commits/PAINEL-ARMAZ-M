...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==="status"){
      const {count,error}=await db.from("product_catalog").select("sku_code",{count:"exact",head:true}).not("factor_hecto_commercial","is",null);
      if(error)throw error;
      return json({count:count||0});
    }
    if(action==="get"){
      const {data,error}=await db.from("product_catalog").select("sku_code,sku_name,factor_hecto_commercial,boxes_per_pallet,source_file,updated_at").not("factor_hecto_commercial","is",null).order("sku_code");
      if(error)throw error;
    ...
    if(action==="import"){
      ...
      for(let i=0;i<items.length;i+=200){
        const batch=items.slice(i,i+200).map((x:any)=>({
          ...
          factor_hecto_commercial:n(x.factor_hecto_commercial),
          boxes_per_pallet:n(x.boxes_per_pallet),
          source_file:String(body.source_file||"01.11"),
          updated_at:new Date().toISOString()
        })).filter((x:any)=>x.sku_code&&x.factor_hecto_commercial&&x.factor_hecto_commercial>0);
