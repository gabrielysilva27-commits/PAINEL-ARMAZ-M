(() => {
  ...
  function render(){
    ...
    const totalSemesterSkus=act?.total_policy_count??d.items.length;
    root.innerHTML='<div class="policy-v4">'+
      ...
      '<section class="policy-rules"><div><small>Mínimo</small><strong>3 dias</strong></div><div><small>Objetivo</small><strong>5 dias</strong></div><div><small>Máximo médio</small><strong>'+esc(avgMax==null?'—':nf.format(avgMax)+' dias')+'</strong></div><div><small>SKUs ativos</small><strong>'+nf.format(totalSemesterSkus)+'</strong></div></section>'+
      ...
      '</tbody></table></section>'+
      '<p class="policy-foot">A política de limites permanece fixa no semestre, mas o universo do OOR considera somente SKUs com puxada nos últimos 30 dias. '+(act?('Base de puxada usada: '+dt(act.window_start)+' a '+dt(act.window_end)+'. '):'')+'SKUs sem puxada nesse período ficam fora do denominador e não são classificados como OUT.</p>'+
      '</div>';
