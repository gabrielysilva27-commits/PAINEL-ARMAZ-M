(() => {
  ...
  async function openOor(){try{if(!window.__stockOor?.open)throw new Error('OOR ainda não carregou. Atualize a página.');await window.__stockOor.open()}catch(e){showToast(String(e.message||e),true)}}
  async function openPolicy(){try{if(!window.__stockPolicy?.open)throw new Error('Política de Estoque ainda não carregou. Atualize a página.');await window.__stockPolicy.open()}catch(e){showToast(String(e.message||e),true)}}
  function putawayAssets(){if(window.__receivingPutaway)return Promise.resolve(window.__receivingPutaway);if(putawayLoading)return putawayLoading;putawayLoading=new Promise((resolve,reject)=>{if(!$('receivingPutawayCss')){const l=document.createElement('link');l.id='receivingPutawayCss';l.rel='stylesheet';l.href='receiving-putaway.css?v=20260921-3';document.head.appendChild(l)}const s=document.createElement('script');s.src='receiving-putaway.js?v=20260921-3';s.async=true;s.onload=()=>resolve(window.__receivingPutaway);s.onerror=()=>reject(new Error('Falha ao carregar Ordem de Guarda'));document.body.appendChild(s)});return putawayLoading}
  async function openPutaway(){try{const m=await putawayAssets();await m.open()}catch(e){showToast(String(e.message||e),true)}}

  function setNavGroupOpen(group,parent,open){
    group.classList.toggle('open',!!open);
    parent.setAttribute('aria-expanded',open?'true':'false');
  }
  function bindNavGroupToggle(group,parent){
    parent.setAttribute('aria-expanded',group.classList.contains('open')?'true':'false');
    parent.onclick=(ev)=>{
      ev.preventDefault();
      ev.stopPropagation();
      setNavGroupOpen(group,parent,!group.classList.contains('open'));
    };
  }
  function mount(){
