...
// 3.2.71: execução exclusiva sobre o Promax já aberto no Edge normal.
...
async function main() {
  ...
  if (process.argv.indexOf("--calibrate") >= 0) {
    ...
    try {
      log("Abrir Promax: usando o Microsoft Edge normal.");
      await promax.openCalibrationBrowser(config, ROOT);
      log("Abrir Promax: URL enviada para o Edge normal.");
