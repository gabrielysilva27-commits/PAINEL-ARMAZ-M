...
async function browserDownload(rootDir, validateCsv) {
  ...
  // 0) Primeiro faça um gesto realmente confiável: coloque foco no botão CSV
  // dentro do frame correto e envie ENTER pelo Windows. Se o Promax abrir o
  // arquivo diretamente no Excel, leia a planilha pelo COM sem depender de
  // download, Save As ou diretório do navegador.
  let enterDiag = "";
  let excelDiag = "";
  try {
    await clickCsv();
  ...
  await sleep(1800);
  // Alguns ambientes do Promax associam o .csv.inf ao Excel e abrem a planilha
  // sem criar um arquivo em Downloads. Capture também depois do onclick legado.
  try {
    const excel = captureExcelWorkbook(rootDir, validateCsv);
...
module.exports = { isConfigured, readinessError, missingSelectors, openCalibrationBrowser, export020501 };
 -or $v -eq '') -and $null -ne $cell.Value2){ $v=[string]$cell.Value2 }",
    "            $vals.Add(('\"'+$v.Replace('\"','\"\"')+'\"'))",
    "          }",
